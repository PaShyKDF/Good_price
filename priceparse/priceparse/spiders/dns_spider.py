...
class DnsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for url in self.start_urls:
            yield SeleniumRequest(
                ...
                wait_time=10,
                wait_until=EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'div.catalog-product.ui-button-widget')),
    ...
    def parse_dns(self, response):
        ...
        for product in product_cards:
            name = product.css('span::text').get('').strip()
            price = product.css('.product-buy__price::text').get('').strip()
            link = product.css('.catalog-product__name.ui-link.ui-link_black::attr(href)').get('')
            yield {
                'name': name,
                'price': price,
                'link': link
        ...
        # Handle pagination
        next_page = response.css('.pagination-widget__page-link_next::attr(href)').get()
        if next_page:
            yield SeleniumRequest(
